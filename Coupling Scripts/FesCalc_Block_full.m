...
%     clearvars -except run pdb ene;
    pdb=char('Villin'); %modify to the name of the relevant PDB file
    ...
    disr = [];
    ppos=[];
    hh9=fopen(cat(2,deblank(pdblist(run,:)),'_struct.txt'),'rt'); %secondary structure prediction file generated using STRIDE
    lkk=fgetl(hh9);
    while lkk > 0
...
save('FEData.mat')
writematrix(dGwtcp, [pdb '_couplings.csv'])